...
export async function generateVeoClip({
  ...
  // v62.72 dropped this 360s → 300s; v62.75 puts it BACK. The hedge's
  // elapsed telemetry measured healthy Kling pro completions at 244-285s —
  // a 300s ceiling would declare a healthy 310s clip stalled and pay a
  // full regeneration for a video that was seconds from done. The hedge
  // (runway-job, 300s) covers the slow tail; this ceiling only exists to
  // call genuinely dead jobs, and Invergordon's real stall sat past 360.
