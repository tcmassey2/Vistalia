...
//
// Daily Runway + OpenAI cost rollup, restricted to admin users (defined
// by the ADMIN_USER_IDS env var, comma-separated Supabase user IDs).
//
...
export default async function handler(request, response) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    const runwayCount = Array.isArray(row.scenes)
      ? row.scenes.filter((s) => !s.wasFallback).length // wasFallback=true → Ken Burns, not Runway
      : sceneCount;
    const runwayCost = row.engine === "runway"
      ? runwayCount * SECONDS_PER_RUNWAY_SCENE * RUNWAY_DOLLARS_PER_SECOND
      : 0;
    const openAiCost = OPENAI_DOLLARS_PER_RENDER;

    if (!byDay[day]) byDay[day] = { date: day, renders: 0, runwayCents: 0, openAiCents: 0 };
    byDay[day].renders++;
  ...
  const monthSpentCents = monthRunwayCents + monthOpenAiCents;
  ...
  return response.status(200).json({
    summary: {
      ...
      totalRenders,
      runwayRenders,
      kenBurnsRenders: totalRenders - runwayRenders,
      totalRunwayCents,
      totalOpenAiCents,
      totalCents: totalRunwayCents + totalOpenAiCents
    },
    currentMonth: {
    ...
    note: "Estimates. Runway: $0.05/generated-second. OpenAI: ~$0.05/render. Reconcile with the actual Runway + OpenAI dashboards monthly."
  });
}
