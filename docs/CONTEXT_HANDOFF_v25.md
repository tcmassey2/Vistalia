...
_Generated June 9, 2026. Paste this entire document into a new Claude chat to bring it up to speed instantly. Last working session was on Cowork with Opus 4.7._
...
We just committed and pushed v25 Phase 1b: **`ba52624`** (pivot from GCP-direct Veo to fal.ai-routed Veo). Production routing is UNCHANGED — Runway Gen-4 Turbo is still the engine all production renders use. The fal.ai-Veo plumbing exists ONLY as a standalone smoke-test endpoint `POST /test/veo` on the render worker.
...
## Why we pivoted (this is the saga, condensed)
...
└── marketing/                    (asset folder)
```

## Things you should NOT do

- **Don't push code.** I push manually. You can `git commit` locally but stop there.
...
## Things you SHOULD know about my preferences

- I work in long sessions (this one ran for several days across multiple compaction cycles)
- I do real work between sessions — assume the codebase may have drifted since you last saw it; check git status before assuming
- I value honest tradeoff articulation — "Option A gives X but costs Y; Option B is opposite" beats "let me think about this"
- I appreciate when you ask one clarifying question via AskUserQuestion before big architectural moves, not five
- I appreciate when you tell me what NOT to do alongside what to do
- I push code; you don't have remote-push access. Always tell me the push command at the end of a commit.
- File creation: use real Edit/Write tools, not chat output. I want files on disk.
- Use the TaskList tool — it renders as a widget in Cowork and helps me track multi-phase work
...
End of context handoff. Greet me, confirm you've read this, ask one clarifying question if any, then we proceed.
